<h1 align="center">Hi 👋, I'm Mohd Raza Khan</h1>
<h3 align="center">🚀 Full-Stack Developer | Flutter | AI | IoT Enthusiast</h3>

---

## 👨‍💻 About Me
- 🎓 Final Year B.Tech CSE @ Sharda University
- 📱 Built **OneRoom App** (100+ downloads, 4.6⭐)
- 🤖 Working on **AI & Voice-based Systems**
- 🚀 Passionate about solving real-world problems through tech
- 🎯 Actively looking for **Full-Stack / Flutter / Backend roles**

---

## 🔥 Featured Projects

### 🚴 OptiRider
> Multi-platform app for gig riders to manage multiple delivery apps in one place  
- 🧠 Reduces app switching & increases efficiency  
- ⚙️ Flutter | Firebase | Automation  

---

### 🏠 OneRoom Application
> Smart roommate & expense management app  
- 📊 Expense tracking, task management  
- ⭐ 100+ Downloads on Play Store  
- ⚙️ Flutter | Firebase  

---

### 🤖 FrontDesk AI Voice Assistant
> AI-powered voice receptionist using LiveKit + Gemini  
- 🎤 Real-time voice interaction  
- 🧠 Smart query handling & escalation  
- ⚙️ AI | WebRTC | Node.js  

---

## 🛠 Tech Stack
- 💻 Languages: Java, Python, JavaScript  
- 📱 Mobile: Flutter  
- 🌐 Web: React.js, Node.js  
- ☁️ Backend & Cloud: Firebase, AWS  
- 🗄️ Databases: MySQL, MongoDB  

---
## 📊 GitHub Stats

<p align="center">
  <table>
    <tr>
      <td>
        <img src="https://github-readme-stats.vercel.app/api?username=mohdrazakhan&show_icons=true&theme=default" />
      </td>
      <td>
        <img src="https://github-readme-streak-stats.herokuapp.com/?user=mohdrazakhan" />
      </td>
    </tr>
  </table>
</p>

---

## 🌐 Connect With Me
- 🌍 Portfolio: https://mohdrazakhan.me  
- 💼 LinkedIn: https://www.linkedin.com/in/mohdrazakhan32/
- 📧 Email: mohdrazakhan32@gmail.com

---

⭐ *Always building, always learning.*
